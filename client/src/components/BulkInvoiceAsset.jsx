...
const BulkInvoiceUpdateAsset = ({ selectedItems, onUpdated, closePopup }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(`${baseURL}/asset/bulk-update`, {
        method: "POST",
    ...
    <div className="popup-overlay">
      <div className="popup-content add-stock-form">
        ...
        <div className="relative" ref={invoiceDropdownRef} style={{ marginBottom: "15px" }}>
          ...
          >
            {invoiceId
              ? invoices.find((inv) => Number(inv.id) === Number(invoiceId))?.invoice_no
              : "Select Invoice"}
            <span>▼</span>
          </button>
...
export default BulkInvoiceUpdateAsset;
